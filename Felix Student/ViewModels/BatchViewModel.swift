...
class BatchViewModel: ObservableObject, Identifiable {
    ...
    func subscribe(facultyUid: String) {
        DatabaseReference.shared.batchReference().whereField(Constants.FACULTYUID, isEqualTo: facultyUid).whereField(Constants.BATCHTYPE, isEqualTo: Constants.ONGOING).getDocuments { (snapshot, error) in
...
//                }))!
    ...
    func getBatchForStudent() {
        DatabaseReference.shared.walkinReference().whereField(Constants.UID, isEqualTo: SessionStore.shared.session?.uid ?? "").getDocuments { (snap, err) in
            if let e = err {
                ...
                if let batchId = student?.batchId {
                    DatabaseReference.shared.batchReference().document(batchId).getDocument { [unowned self] (snapshot, error) in
                        if let err = error {
                            print("Error getting documents: \(err)")
                        } else {
